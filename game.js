...
const VERSION = "v0.4.2-home-mobile-rebuild-draft-fix1";
...
const SAVE_VERSION = "v0.4.2-home-mobile-rebuild-draft-fix1";
let uiMode = "title";
...
function renderHomeGuidePanel() {
  ...
  const icon = tutorial ? "💡" : (suggestion.icon || "🎸");
...
function renderHomePrimaryActions() {
  ...
  return `<section class="home-primary-grid" aria-label="主操作">
    <button class="jumpTabBtn home-main-action home-main-command big-action ${commandLocked ? "locked" : ""}" data-view="command" ${commandLocked ? "disabled" : ""}>
      <span>⚡</span><b>今週の行動</b><small>${escapeHtml(commandNote)}</small><em>${commandLocked ? "ロック" : "進む"}</em>
    </button>
    <button class="openSongEditorBtn home-main-action home-main-song big-action ${songLocked ? "locked" : ""}" ${songLocked ? "disabled" : ""}>
      <span>🎼</span><b>作詞・作曲</b><small>${escapeHtml(songNote)}</small><em>${escapeHtml(song.badge)}</em>
    </button>
  </section>`;
}
...
function renderHomeUtilityTiles() {
  const tiles = [
    ...
    bandSystemOn() ? homeMenuButton("bandbook", "🎤", "バンド図鑑", `${bandDiscoveredCount()}/${Object.keys(BAND_DATABASE).length}組`) : homeMenuButton("bandbook", "🎤", "バンド図鑑", "交流記録"),
    homeMenuButton("library", "📚", "記録", "歴代・ログ"),
    homeMenuButton("shop", "🛒", "ショップ", "機材・回復"),
    homeMenuButton("log", "📺", "ログ", "履歴")
  ].join("");
  const dev = devModeOn() ? `<div class="home-dev-row">${homeMenuButton("dev", "🛠️", "DEV", "確認")}</div>` : "";
...
function renderCancelLiveConfirmOverlay() {
  ...
  return `<div class="modal-backdrop"><div class="event-modal warn">
    ...
    <div class="modal-copy"><h2>${noShow ? "ライブをドタキャンする？" : "ライブをキャンセルする？"}</h2>
      <p>${ev && v ? `${ev.turn}ターン目「${escapeHtml(v.name)}」を${noShow ? "ドタキャン" : "キャンセル"}します。` : "対象のライブをキャンセルします。"}<br>${noShow ? `${escapeHtml(feeLabel)}＋損料 ${penalty.toLocaleString()}円が発生し、信頼度と知名度が大きく下がります。` : "メリットはなく、信頼度と知名度が少し下がります。"}</p>
    </div>
...
function renderBoostSongConfirmOverlay() {
  ...
  return `<div class="modal-backdrop"><div class="event-modal event">
    <div class="modal-icon">📈</div>
    <div class="modal-copy"><h2>この曲を強化する？</h2><p>「${escapeHtml(song.title)}」 / ${escapeHtml(genreDisplay(song))}<br>現在：強化Lv${song.boostLevel || 0}<br>疲労+${cost}%</p><p>疲労を消費して、曲の強化Lvと演奏力を少し上げます。</p>${over ? `<p class="bad-text">疲労が100％を超えるため、今は強化できません。</p>` : ""}</div>
...
function bindEvents() {
  ...
  document.querySelectorAll(".prepStepBtn, .prepStepNextBtn, .prepStepPrevBtn, .prepStepJumpBtn").forEach(btn => btn.addEventListener("click", () => { state.livePrepStep = clamp(Math.floor(Number(btn.dataset.step || 1)), 1, 5); render(); }));
  ...
  document.querySelectorAll("#storyNextBtn").forEach(btn => btn.addEventListener("click", advanceStoryEvent));
...
疲労+${fatigueGainNow}%`, type:"event", icon:"🍻" };
      }
    } else {
...
// v0.4.2-home-mobile-rebuild-draft-fix1: home/phone/opening UI rebuild layer.
...
function renderMainContent() {
  ...
  const homeBack = (state.view || "home") !== "home" ? `<button class="jumpTabBtn page-home-btn v042-page-home" data-view="home">← ホームへ</button>` : "";
...
function renderPlanBlockV042() {
  ...
  return `<button class="jumpTabBtn v042-plan" data-view="schedule"><span>📅</span><b>予定</b><small>${next ? `${next.turn}T / あと${remain}T` : "予定なし"}</small></button>`;
}
...
function renderHomeStatusGridV042() {
  const b = state.band || {};
  const rows = [
    ["資金", v042FormatMoney(b.funds), Number(b.funds || 0) < 0 ? "bad" : Number(b.funds || 0) < 10000 ? "warn" : "good", "💴"],
    ["ファン", `${Math.round(b.fans || 0)}人`, "good", "👥"],
    ["疲労", `${Math.round(b.fatigue || 0)}%`, Number(b.fatigue || 0) >= 80 ? "bad" : Number(b.fatigue || 0) >= 60 ? "warn" : "good", "⚡"],
    ["知名度", Math.round(b.fame || 0), "info", "📣"],
...
function renderIntroScreen() {
  ...
  document.getElementById("openingNextBtn")?.addEventListener("click", () => { state.openingStep = Math.min(9, step + 1); render(); });
  document.getElementById("openingPlayerNameBtn")?.addEventListener("click", () => {
    const name = cleanProfileInput(document.getElementById("openingPlayerNameInput")?.value, "", 24);
    if (!name) { alert("主人公名を入力してください。"); return; }
    state.pendingPlayerName = name;
  ...
  document.getElementById("openingFinishBtn")?.addEventListener("click", () => {
    state.introSeen = true;
    state.bandNameFinalized = true;
    state.bandNamePrompt = false;
